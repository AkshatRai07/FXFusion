...
contract fEUR is ERC20, Ownable {
    address public appContract;
    uint256 public constant SCALE = 1e18;

    constructor() ERC20("flowEUR", "fEUR") Ownable(msg.sender) {
        _mint(address(this), 10 * 10 ** 18 * 10 ** decimals()); // 10 quintillion fEUR
    }
    ...
    function buyTokens(uint256 rate) external payable onlyAuthorized {
        require(msg.value > 0, "Send ETH to buy fEUR");

        uint256 tokenAmount = (msg.value * rate) / SCALE;
        require(balanceOf(address(this)) >= tokenAmount, "Not enough fEUR in contract");
